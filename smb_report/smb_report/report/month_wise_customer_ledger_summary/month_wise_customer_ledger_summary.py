...
def execute(filters=None):
	...
	for month_num in range(1,13):
		...
		month_start_date = today.replace(day=1, month=month_num, year=int(filters.fiscal_year))
		month_end_date = frappe.utils.get_last_day(month_start_date)
		filters.from_date = month_start_date
		filters.to_date = month_end_date
		columns, data = PartyLedgerSummaryReport(filters).run(args)
		month_key = str(month_num)
		for d in data:
			d["month"] = month_map[month_key]
			final_data.append(d)
